# 📁 Project Structure

```bash
.
├── hypr
│   └── hyprland.conf
├── keyd
│   └── default.conf
├── nvim
│   ├── 02_23_অপরাহ্ণ_07_08_25.mp4
│   ├── init.lua
│   ├── lazy-lock.json
│   ├── lua
│   │   └── sj
│   │       ├── core
│   │       │   ├── custom
│   │       │   │   ├── checking2.lua
│   │       │   │   ├── checking-feature.lua
│   │       │   │   ├── CustomMode
│   │       │   │   │   ├── init.lua
│   │       │   │   │   └── ScrollMode.lua
│   │       │   │   ├── Git
│   │       │   │   │   ├── autoPush2.lua
│   │       │   │   │   ├── autoPush.lua
│   │       │   │   │   ├── clone.lua
│   │       │   │   │   ├── github2.lua
│   │       │   │   │   ├── githubDescription.lua
│   │       │   │   │   ├── github.lua
│   │       │   │   │   ├── githubRepoGenerateAndPush.lua
│   │       │   │   │   ├── init.lua
│   │       │   │   │   ├── readmeGen2.lua
│   │       │   │   │   ├── readmeGenerate.lua
│   │       │   │   │   └── smartCopyPaste.lua
│   │       │   │   ├── html.lua
│   │       │   │   ├── init.lua
│   │       │   │   ├── insertMode.lua
│   │       │   │   ├── JsTsPro
│   │       │   │   │   ├── init.lua
│   │       │   │   │   ├── jsTs2.lua
│   │       │   │   │   ├── jsTs.lua
│   │       │   │   │   ├── jsxPro2.lua
│   │       │   │   │   ├── jsxPro.lua
│   │       │   │   │   └── reBuildModule.lua
│   │       │   │   ├── normalModeInInrsert.lua
│   │       │   │   ├── others.lua
│   │       │   │   └── TmuxZshCli
│   │       │   │       ├── build.lua
│   │       │   │       ├── fileUtils.lua
│   │       │   │       ├── init.lua
│   │       │   │       ├── nvimTerminal.lua
│   │       │   │       ├── tmuxCommandSender.lua
│   │       │   │       ├── tmuxPaneContent.lua
│   │       │   │       ├── tmuxZsh2.lua
│   │       │   │       └── tmuxZsh.lua
│   │       │   ├── init.lua
│   │       │   ├── keymaps.lua
│   │       │   ├── options.lua
│   │       │   └── utils.lua
│   │       ├── lazy.lua
│   │       └── plugins
│   │           ├── alpha.lua
│   │           ├── autopairs.lua
│   │           ├── autoSave.lua
│   │           ├── auto-session.lua
│   │           ├── blammer.lua
│   │           ├── bufferline.lua
│   │           ├── chatgpt.lua
│   │           ├── colorscheme.lua
│   │           ├── comfy-line-numbers.lua
│   │           ├── comment.lua
│   │           ├── copilotChat.lua
│   │           ├── cursor-line.lua
│   │           ├── dressing.lua
│   │           ├── emoji.lua
│   │           ├── error-lens.lua
│   │           ├── fire-nvim.lua
│   │           ├── flash.lua
│   │           ├── formatting.lua
│   │           ├── fzf-lua.lua
│   │           ├── gh.lua
│   │           ├── gitsigns.lua
│   │           ├── gx.lua
│   │           ├── hop.lua
│   │           ├── hurl.lua
│   │           ├── indent-blankline.lua
│   │           ├── init.lua
│   │           ├── lazygit.lua
│   │           ├── linting.lua
│   │           ├── lsp
│   │           │   ├── lspconfig.lua
│   │           │   └── mason.lua
│   │           ├── lualine.lua
│   │           ├── lua-snip.lua
│   │           ├── mason-workaround.lua
│   │           ├── md-pdf.lua
│   │           ├── multi-cursor.lua
│   │           ├── neotree.lua
│   │           ├── noice.lua
│   │           ├── nvim-cmp.lua
│   │           ├── nvim-tree.lua
│   │           ├── rainbow-matching.lua
│   │           ├── snacks.lua
│   │           ├── supermaven.lua
│   │           ├── surround.lua
│   │           ├── tailwind.lua
│   │           ├── tailwind-tools.lua
│   │           ├── telescope.lua
│   │           ├── time_tracker.lua
│   │           ├── tiny_glimmer.lua
│   │           ├── todo-comments.lua
│   │           ├── toggleTerm.lua
│   │           ├── treesitter.lua
│   │           ├── trouble.lua
│   │           ├── vim-maximizer.lua
│   │           ├── web-tools.lua
│   │           ├── which-key.lua
│   │           ├── yanky.lua
│   │           └── yazi.lua
│   ├── readmeGenerateFull.md
│   ├── README.md
│   └── structure.md
├── README.md
├── tmux
│   ├── customBingdings.conf
│   ├── plugins.conf
│   ├── prefix-mappings.conf
│   ├── statusline.conf
│   └── visuals.conf
├── tmux.conf
├── tmuxModularized
│   └── customBingdings.conf
├── yazi
│   ├── keymap.toml
│   ├── refractorYaziPrompt.md
│   ├── theme.toml
│   └── yazi.toml
├── zshrc
└── zshScript
    ├── alias.sh
    ├── browserAutomation
    │   ├── activate
    │   ├── close_linkedin_tabs.py
    │   ├── close_youtube_tabs.py
    │   ├── howTowork.md
    │   ├── reload_except_github.py
    │   ├── reload_other_tabs.py
    │   ├── reloadOtherTabs.ts
    │   └── structure.md
    ├── config
    ├── copyLastCommandAndOutput.sh
    ├── customFunction.sh
    ├── enviromentVariable.sh
    ├── fzf-repo-manager-level1.sh
    ├── fzf-repo-manager-level2.sh
    ├── fzf-repo-manager.sh
    ├── open-nvim-and-sync-panes.sh
    ├── package.json
    ├── package-lock.json
    ├── ranger
    │   ├── commands_full.py
    │   ├── commands.py
    │   ├── rc.conf
    │   ├── rifle.conf
    │   ├── scope.sh
    │   └── scripts
    │       └── tmux_sync.sh
    ├── README.md
    ├── scriptforAutokey
    │   ├── chatgptOpen.py
    │   ├── chk3.py
    │   ├── copiedNotification.py
    │   ├── DateAndTime.py
    │   ├── email.py
    │   ├── gmailOpen.py
    │   ├── helpWebDev.py
    │   ├── Insert Date.py
    │   ├── openchatGpt.py
    │   ├── open_terminal.py
    │   ├── phoneNumber.py
    │   ├── ph.py
    │   ├── quranPsycologyYahiaAmin.py
    │   ├── salam.py
    │   ├── spAll.py
    │   └── sp.py
    ├── shellScript
    │   ├── backup_dotfiles.sh
    │   ├── full_dotfile_push.sh
    │   ├── lf_fm_shell_script
    │   │   └── dev-launch.sh
    │   ├── rangerShellScript
    │   │   └── ranger-file-picker-setup.sh
    │   ├── rofi-cliphist.sh
    │   ├── sessionTracker.sh
    │   ├── switch-to-ranger.sh
    │   ├── tmuxCopy2.sh
    │   └── tmux-copy-pwd-and-nvim.sh
    ├── structure.md
    ├── surfingkeys.bundle.js
    ├── surfingModule
    │   └── perplexity.js
    ├── surfingTempo.js
    └── tmuxModular
        └── plugins.conf

27 directories, 173 files

```
